...
addPair = function(l1, l2) {
    ...
    pair.save(function(err){
        if (err) {
            ...
            console.log(('[DB] Successfully saved new pair ' + pair.alpha + ', ' + pair.omega).green);
...
exports.terminateUser = function(number) {
    Pair.findOne({$or: [{'alpha':number},{'omega':number}]}, function(err, pair) {
        // User not found
        if (err) {
            console.log(('[DB] Error while getting user to terminate: ' + err).red);
        // Else, returning user
        } else if (pair) {
            console.log(('[DB] Terminate: ' + number).yellow);
            var left_over;
            var to_remove;
